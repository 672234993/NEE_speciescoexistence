...
# R-code associated with the paper "The spatial scales of species coexistence" by:
# Simon Hart, Jacob Usinowicz, and Jonathan M. Levine
# published in: Nature Ecology and Evolution


################################################################################
#
# The code enables a spatially explicit simulation of N species competing in a 
# heterogeneous environment, numerical analysis of coexistence, and a simulated 
# sampling scheme to reproduce coexistence-area and species-area curves. 
# 
# Phase 1 generates an equilibrium community, Phase 2 tests the coexistence of each 
...
# community in areas of increasingly large size and tests coexistence of the 
# subsampled community.
#
# The model is a discrete space lottery model, where multiple populations
# may occupy the same site in varying proportions. The model allows 
...
#
# The implementation is currently set for short-range dispersal (and competition), 
# and long range spatial correlation. See section "Variables of model that can be 
# tuned" to change these and other parameter values. 
...
#Final object is two matrices where entries in matrix 1 correspond to the x 
#coordinate of each site, and matrix 2 corresponds to y coordinates of each site. 


#Population matrices. Each population gets to save only 2 time points 
